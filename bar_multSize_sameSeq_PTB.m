...
%
% TCS April 2021 - defaults to a continuous-width bar, rather than 3
% discrete values; centered stimulus in any screen aperture; and adjusted
% for 750 ms TR (10.5 s blank to start/end)
...
if p.scanner == 1
    p.start_wait = 14*p.TR; % s - time after trigger (10.5 s for 750 ms TR)
    p.end_wait   = 14*p.TR; % s - time after last bar sweep (10.5 s for 750 m TR)
else
    p.start_wait = 1;
    p.end_wait = 1;
end
