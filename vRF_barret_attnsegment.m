% vRF_barret_attnsegment.m
%
%
% DDT, KL, TCS 11/25/2020
% adapted from bar_multSize_sameSeq_PTB.m 
%
...
% 1.2 s TR: 304 TRs
% 
% TODO: remember to change eyetracking when running exp



function vRF_barret_attnsegment(subj,run,seq)
...
p.coh_sample  = 1; % coherence of sample stimulus (outer bars)
...
p.segment_gap = 0.5; % deg, gap between segments - determines SIZE of rect, will be centered on a known point, so will be gap/2 at top/bottom/left/right too
...
for i = 1:p.n_segments
    p.segment_dirs(:,i) = randsample([0 1 -1],size(p.segment_dirs,1),'true')';
end
...
% remember to change these when running on scanner laptop
p.resp_keys = [KbName('1!'),KbName('2@')]; % 1 = left/up, 2 = right/down
p.start_key = KbName('5%'); 
...
% --------- eyetracking ----------- %
p.do_et = 0; % remember to change this when running actual exp
